...
{* SalesPlatform.ru begin: Fixed select user button *}
                                        &nbsp;<img title="{$APP.LBL_CHANGE_TITLE}" src="{'select.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_CHANGE}" class="small" name=btn1 align="absmiddle" LANGUAGE=javascript onclick='return window.open("index.php?module=Users&action=Popup&html=Popup_picker&form=vtlibPopupView&form_submit=false&fromlink={$fromlink}&recordid={$ID}&forfield={$fldname}","test","width=640,height=603,resizable=0,scrollbars=0");' style='cursor:hand;cursor:pointer'>
