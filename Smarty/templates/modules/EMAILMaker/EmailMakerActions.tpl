  ...
  {if $CRM_TEMPLATES_EXIST eq '0'}
        <tr>
  		<td class="rightMailMergeContent"  style="width:100%;">
			<select name="use_common_email_template" id="use_common_email_template" class="detailedViewTextBox" {* if $PICKLIST_COUNT neq '1'}multiple{/if *} style="width:90%;" size="{$PICKLIST_COUNT}">
            {html_options  options=$CRM_TEMPLATES selected=$DEFAULT_TEMPLATE}  
            </select>        
  		</td>
		</tr>
  {/if}
