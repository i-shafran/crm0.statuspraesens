...
<tr>
	<td style="padding-left:10px;padding-right:50px" class="moduleName" nowrap>{$APP.$CATEGORY} > <a class="hdrLink" href="index.php?action=index&module=EMAILMaker&parenttab={$CATEGORY}">{$MOD.LBL_TEMPLATE_GENERATOR}</a></td>
			...
			<td>
				...
				<tr>
					{if $EDIT eq 'permitted'}
	                    <td style="padding-right:0px;padding-left:10px;"><a href="index.php?module=EMAILMaker&action=SelectEmailTemplates&return_action=DetailView&parenttab={$CATEGORY}"><img src="{$IMAGE_PATH}btnL3Add.gif" alt="{$MOD.LBL_ADD_TEMPLATE}" title="{$MOD.LBL_ADD_TEMPLATE}" border=0></a></td>
					{else}
		...
		<td class="small">
			<!-- All Menu -->
				...
				{if $CHECK.moduleSettings eq 'yes'}
	        		<td style="padding-left:10px;"><a href='index.php?module=Settings&action=ModuleManager&module_settings=true&formodule=EMAILMaker&parenttab=Settings'><img src="{'settingsBox.png'|@vtiger_imageurl:$THEME}" alt="{$MODULE|getTranslatedString:$MODULE} {$APP.LBL_SETTINGS}" title="{$MODULE|getTranslatedString:$MODULE} {$APP.LBL_SETTINGS}" border="0"></a></td>
				{/if}
				</tr>
				</table>
		</td>			
