...
<table cellpadding=0 cellspacing=0 border=0 class="small" width="98%">
    ...
    <tr>
        ...
        <tr>
            {*SalesPlatform.ru begin support UTF-8*}
            <td>
                <a class="mm_folder" id='_mailfolder_{$FOLDER->name()|@htmlentities:null:"UTF-8"}' href='#{$FOLDER->name()|@htmlentities:null:"UTF-8"}' onclick="MailManager.clearSearchString(); MailManager.folder_open('{$FOLDER->name()|@htmlentities:null:"UTF-8"}'); "
                >{if $FOLDER->unreadCount()}<b>{$FOLDER->name()|@htmlentities:null:"UTF-8"} ({$FOLDER->unreadCount()})</b>{else}{$FOLDER->name()|@htmlentities:null:"UTF-8"}{/if}</a>
            </td> 
           {* <td>
