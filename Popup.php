...
switch($currentModule)
{
	...
	case 'Vendors':
		$smarty->assign("SINGLE_MOD",'Vendor');
                // SalesPlatform.ru begin Assign RETURN_MODULE for Vendors
                if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] !='')
                    $smarty->assign("RETURN_MODULE",vtlib_purify($_REQUEST['return_module']));
		else
                    $smarty->assign("RETURN_MODULE",'Emails');
...
// SalesPlatform.ru begin: Added select possibility for contacts activities
        if($currentModule == 'Calendar')
            $where_relquery .= " and vtiger_activity.activitytype!='Task'";
// SalesPlatform.ru end
...
if (vtlib_isModuleActive('SPPayments')) {
    $smarty->assign("SPPAYMENTS_ACTIVE", true); 
}
// SalesPlatform.ru end
