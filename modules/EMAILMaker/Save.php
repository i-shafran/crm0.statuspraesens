...
if (count($Send_Emails) > 0)
{
    foreach ($Send_Emails AS $SED)
    {
        ...
        $result = $adb->pquery($sql,array($from_name,$from_email,$SED["subject"],$SED["description"],'1',$ids_for_pdf,$pdf_template_ids,$pdf_language,$current_user->id,$SED["att_documents"],$drip_group_id,$SED["drip_delay"],$SED["drip_delay"],"0",$related_to,$pmodule));
