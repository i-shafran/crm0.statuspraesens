...
if($return_id != '')
{
    if ($return_module != '' && (isPermitted($return_module, "EditView", $return_id) == 'yes')) {
        require_once ("modules/$return_module/$return_module.php");
        $return_focus = new $return_module();
        $return_focus->id = $return_id;
        $return_focus->retrieve_entity_info($return_id, $return_module);
        if ($return_module == 'Accounts' || $return_module == 'Contacts'
                || $return_module == 'Vendors') {
            $focus->column_fields['payer'] = $return_focus->column_fields['record_id'];
        }
        if ($return_module == 'Invoice' || $return_module == 'SalesOrder'
                || $return_module == 'PurchaseOrder') {
            $focus->column_fields['related_to'] = $return_focus->column_fields['record_id'];
            $focus->column_fields['amount'] = $return_focus->column_fields['hdnGrandTotal'];
            if ($return_module == 'PurchaseOrder') {
                $focus->column_fields['payer'] = $return_focus->column_fields['vendor_id'];
            } else {
                $focus->column_fields['payer'] = $return_focus->column_fields['account_id'];
            }
        }
    }
}
