...
class MailManager_Connector {
	...
	function folders($ref="{folder}") {
		...
		foreach($result as $row) {
			...
			$folders[] = $this->folderInstance(str_replace($ref, "", $this->convertCharacterEncoding($row->name, "UTF8", "UTF7-IMAP")));
	...
	function updateFolder($folder, $options) {
		...
		$mailbox = $this->convertCharacterEncoding($folder->name($this->mBoxUrl), "UTF7-IMAP","UTF8"); //Encode folder name
	...
	function getFolderList() {
		if(!empty($this->mBoxBaseUrl)) {
			...
			if (is_array($list)) {
				foreach ($list as $val) {
                    // SalesPlatform.ru begin: UTF8 support
					$folder = $this->convertCharacterEncoding( $val, 'UTF8', 'UTF7-IMAP' ); //Decode folder name
