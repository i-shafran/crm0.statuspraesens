		...
		if ('open' == $request->getOperationArg()) {
			...
			if (!empty($to_string)) {
				...
				foreach($toArray as $to) {
					...
					$mailer->IsHTML(true);
                                        // SalesPlatform.ru begin: Fixed buggy reply to
			...
			if (MailManager_Utils::allowedFileExtension($attachmentName)) {
                ...
                if($attachment[$attachmentName]) {
                    ...
                    // SalesPlatform.ru begin added addslashes
                    header("Content-Disposition: attachment; filename=\"".addslashes($attachmentName)."\"");
