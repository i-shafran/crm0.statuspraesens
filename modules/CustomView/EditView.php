...
if($recordid == "") {
	...
	if($oCustomView->isPermittedCustomView($recordid,$now_action,$oCustomView->customviewmodule) == 'yes') {
		...
		// SalesPlatform.ru begin: Translate filter values before output
        for($i = 1; $i <= count($advfilterlist); $i++) {
           foreach($advfilterlist[$i]['columns'] as $column_number => $criteria) {
				$filterValues = explode(",",$criteria["value"]);
		
				for ($j = 0; $j < count($filterValues); $j++) {
					$filterValues[$j] = getTranslatedString( trim($filterValues[$j]), $cv_module );
				}
				
				$advfilterlist[$i]['columns'][$column_number]["value"] = implode(", ", $filterValues);
			}
		}
